...
| 32 | "You're done for today" end state; no infinite scroll, ever | 4 | 5 | 3 | 5 | 300 | open |
...
| 106 | Reader stop card: you're done for today after one brief + recall | 4 | 5 | 3 | 5 | 300 | open |
...
### Selection
- Top scores: #25 (400), then #5/#51/#52/#102 at 320. #102 shares the evidence-drawer surface with #25 → merged into F1. Among remaining 320s, #5 has F=5 and compounds with the recall loop shipped in cycle 001; #51 is the highest-reach new brief with a ready responder merge (#63). #52 deferred to cycle 003.
- **F1 Evidence you can scan** = #25 + #102. Same surface: evidence drawer + `content.js` source objects. Rationale: readers cannot weight a claim without knowing what kind of source sits under it; badges make type visible on the next page load.
- **F2 Explain it back** = #5 + #4 + #110. Same surface: reader end state + recall drawer. Rationale: free-recall grading plus an ordered chain rebuild closes the loop that cycle 001 opened with cues alone.
- **F3 Sleep brief** = #51 + #63. Same surface: `content.js` article + optional printable checklist slide/tool. Rationale: highest R×D open brief; shift and heat sleep loss is a force-multiplier for every other crisis topic already on the site.

## Implementation plan

### F1 Evidence you can scan
- Extend each `sources[]` entry with optional `kind: "guidance"|"peer-reviewed"|"reporting"|"dataset"|"method"|"principle"|"context"|"limits"|"qualifies"|"supports"` (map existing `relation` where needed; display badge from `kind` or a new `evidenceType` field).
- `evidenceMarkup`: render `<span class="evidence-type">` from type map; CSS pill.
- Verify: every source on sudan-access shows a non-empty type label; unknown types fall back to relation string.

### F2 Explain it back
- After last slide (or inside recall drawer): textarea two-sentence explanation; compare token-overlap lightly against `article.summary` (no network); self-grade.
- Chain rebuild: render shuffled `chain` steps as buttons; user orders them; grade exact sequence.
- Verify: explanation box appears on final slide; chain rebuild accepts correct order as Got it and writes recall state.

### F3 Sleep brief
- New article `sleep-infrastructure` in `content.js`, five slides, sources primary (WHO, CDC/NIOSH or equivalent), `reviewed` today.
- Include shift-work / heat / light chain; slide or appendix checklist from #63 as chain or sources note.
- Wire into story grid + optional dispatch; export briefs.json picks it up automatically.
- Verify: article opens, five slides, `grep -c sleep-infrastructure content.js` ≥ 1; briefs export count 8.
